"""CLI layer for FastAPI JWT.

CLI-only (argparse + I/O) and excluded from coverage criterion
G-13. Still type-annotated per project standards.
Uses ``uvicorn`` to serve :data:`fastapi_jwt.app.app`.
"""
...
def parse_args(argv: list[str] | None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="fastapi-jwt",
        description="FastAPI JWT auth (basicthon #17) — run dev server.",
    )
    parser.add_argument(
        "--host",
        default=None,
...
def main(argv: list[str] | None = None) -> None:
    """Entry point for console_scripts and ``python -m fastapi_jwt``."""
    ...
    uvicorn.run("fastapi_jwt.app:app", host=host, port=port, reload=reload_flag)
